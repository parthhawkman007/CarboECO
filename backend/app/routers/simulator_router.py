...
def run_simulation(
    ...
    #   "switch_to_ev": bool,
    #   "ev_annual_km": float,
    #   "install_solar_panels": bool,
    #   "solar_capacity_kwh_annual": float,
    #   "meatless_days_per_week": int, # 0 to 7
    #   "reduce_flight_hours": float
    # }
    ...
    if inputs.get("install_solar_panels", False):
        ...
        savings = kwh * settings.EF_ELECTRICITY_GRID
        co2_saved += savings
    ...
    if days > 0:
        days = min(days, 7)
        # assuming a user eats 0.25 kg of beef/meat on a meat day on average
        savings = days * 52 * (settings.EF_FOOD_BEEF - settings.EF_FOOD_VEGETARIAN) * 0.25
        co2_saved += savings

    # 4. Flight reduction: savings = hours * 1000km/hr * short_flight_factor
    flight_hours = float(inputs.get("reduce_flight_hours", 0.0))
    if flight_hours > 0:
        savings = flight_hours * 800.0 * settings.EF_FLIGHT_SHORT # assuming average 800km/hr speed
        co2_saved += savings
...
def get_simulation_history(
    ...
    sims = db.query(SimulationRun).filter(
